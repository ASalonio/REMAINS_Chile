...
interface = function(land, params){
  ...
  land.utm = left_join(land, dplyr::select(orography, cell.id, utm), by="cell.id") 
  ...
  land.utm = left_join(land.utm, dplyr::select(dta, utm, inter.name), by="utm") %>% dplyr::select(inter.name)
  gc(verbose = FALSE)
