...
#' Chile values of the model's parameters
#'
#' @name default.params
#'
...
params.chile = function(){
  return(list(
    ...
    time.step = 1,
    cell.size = 1000,
    ha.cell = 100, # cell size in ha
    tseq.save.land = seq(0,50,5),
    tseq.out.maps = seq(0,20,10), #3 mapas: al inicio, a los 10 años y a los 20 años. lo cambiamos de 50 a 20 para que me haga un mapa de incendios simulados cada 10 años
    outputs.suffix = "",
    
    ## Land-cover types
    land.cover.type = data.frame(lct.id = 1:9, lct = c("crop", "pine", "oak", "shrub",
    ...
    lct.fire.prone = data.frame(lct = c("crop", "pine", "oak", "shrub",
                                        "water", "grass", "urban", "shrub.to.pine", "shrub.to.oak"),
                                flam = c(0.29, 0.90, 0.52, 0.94, 0, 0.29, 0, 0.94, 0.94)),
    ...
    fire.suppression = data.frame(region = c("Ñuble", "Biobío", "La Araucanía"), fuel.th = c(0,0,0), mosaic.th = c(Inf,Inf,Inf)),
    ...
    rewilding.th = 5, # Minimum years after any transition to shrubs become oak forest through natural succession
    rate.afforest.oak = 0.27, # Annual rate in 100% of shrub converting to oak (en chile 3%, 11 años, 0,27% anual de forma pasiva)
    rate.afforest.pine = 3.65, # Annual rate in 100% of shrub converting to pine (en chile 17%, 5 years, 3.65% annually)
    radius.afforest.oak = 1414, # Radius in m around a shrub location to find out oaks for the afforestation process
    radius.afforest.pine = 16279, # Radius in m around a shrub location to find out oaks for the afforestation process
