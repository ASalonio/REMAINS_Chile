...
land.dyn.mdl.chile = function(scenDir, is.land.cover.change = TRUE, is.wildfire = TRUE, is.prescribed.burn = TRUE, 
  ...
  cat("\nB. Simulations ...\n")
  for (irun in 1:nrun) {
    cat("Starting replicate", irun, "/", nrun, "\n")
    ...
    for (t in time.seq) {
      ...
      if (is.land.cover.change & t %in% lchg.schedule) {
        ...
        land$interface = interface(dplyr::select(land, cell.id, lct), params)
        ...
        aux = land %>% filter(tschg == 0 & trans.type %in% c("SmartPlant", "AgriConver", "RuralAbnd", 
          ...
          left_join(dplyr::select(orography, cell.id, region), by = "cell.id") %>% 
      ...
      if (is.wildfire & t %in% fire.schedule) {
        fires = wildfires_chile(land, params, out.maps, verbose = F)
        ...
        if (out.maps & t %in% params$tseq.out.maps) {
          cat("Generating maps for t =", t, "in tseq.out.maps:", params$tseq.out.maps, "\n")
          ...
          if (length(fire_risk_values) != nrow(dta)) {
            warning("Length mismatch in fire.risk. Adjusting to match dta.")
            fire_risk_values = rep(NA, nrow(dta))  # Temporary fix
            fire_risk_values[dta$cell.id %in% land$cell.id] = fire.risk(land, orography, params$ha.cell)[1:min(length(fire_risk_values), length(fire.risk(land, orography, params$ha.cell)))]
          }
          dta$fire.risk = fire_risk_values
          out.raster[] = dta$id
          terra::writeRaster(out.raster, filename = paste0(scenDir, "/maps/wildfire.ids_r", irun, "t", t, ".tif"), overwrite = TRUE)
          out.raster[] = dta$step
          terra::writeRaster(out.raster, filename = paste0(scenDir, "/maps/wildfire.step_r", irun, "t", t, ".tif"), overwrite = TRUE)
          out.raster[] = dta$fire.risk
          terra::writeRaster(out.raster, filename = paste0(scenDir, "/maps/firerisk_r", irun, "t", t, ".tif"), overwrite = TRUE)
          ...
          terra::writeRaster(out.raster, filename = paste0(scenDir, "/maps/land.cover.type_run", irun, "time", t, ".tif"), overwrite = TRUE)
          cat("Maps generated successfully for t =", t, "\n")
        }
      }
      ...
      if (is.prescribed.burn & t %in% pb.schedule) {
        ...
        if (length(pb$burnt.cells) > 0) {
          ...
          if (out.maps & t %in% params$tseq.out.maps) {
            ...
            out.raster[] = dta$id
            terra::writeRaster(out.raster, filename = paste0(scenDir, "/maps/pburns_r", irun, "t", t, ".tif"), overwrite = TRUE)
          }
        }
      }
    ...
    ## Write outputs for this replicate if requested
    if (write.outputs) {
    ...
    cat("Completed replicate", irun, "/", nrun, "\n")
  }
